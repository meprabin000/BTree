...
public class BTreeFile extends IndexFile implements GlobalConst
{
	...
	*/
	...
	public void destroyFile() throws IOException, IteratorException, UnpinPageException, FreePageException, DeleteFileEntryException, ConstructPageException, PinPageException
	{
	...
	public void insert(KeyClass key, RID rid) throws KeyTooLongException, KeyNotMatchException, LeafInsertRecException, IndexInsertRecException, ConstructPageException,
			UnpinPageException, PinPageException, NodeNotMatchException, ConvertException, DeleteRecException, IndexSearchException, IteratorException, LeafDeleteException,
			InsertException, IOException 
	{
		// [ASantra: 1/7/2023] Write your code here

		//Checking if the header page exists
		...
		if(headerPage.get_rootId().pid==-1)
		{
			...
			root = new BTLeafPage(headerPage.get_keyType());   // Creating root page
			
			rootID = root.getCurPage();  		// Finding root's PageId

			//emptyID.pid = INVALID_PAGE;
		
			pinPage(rootID);		// pinnning the page for ...
	...
	private KeyDataEntry _insert(KeyClass key, RID rid, PageId currentPageId) throws PinPageException, IOException, ConstructPageException, LeafDeleteException, ConstructPageException, DeleteRecException, IndexSearchException, 
			UnpinPageException, LeafInsertRecException, ConvertException, IteratorException, IndexInsertRecException, KeyNotMatchException, NodeNotMatchException, InsertException
	{
		...
		else if(newBasePage.getType() == NodeType.INDEX)
		{
			...
			else
			{
				...
				else
				{
					...
					for(temp = newSplitIndex.getFirst(delRid); newSplitIndex.available_space()< newIndex.available_space(); temp = newSplitIndex.getFirst(delRid))
					{
	...
	public boolean Delete(KeyClass key, RID rid) throws DeleteFashionException, LeafRedistributeException, RedistributeException, InsertRecException, KeyNotMatchException, UnpinPageException,
			IndexInsertRecException, FreePageException, RecordNotFoundException, PinPageException, IndexFullDeleteException, LeafDeleteException, IteratorException, ConstructPageException, DeleteRecException, 
	...
	private boolean NaiveDelete(KeyClass key, RID rid) throws LeafDeleteException, KeyNotMatchException, PinPageException, ConstructPageException, IOException, UnpinPageException, PinPageException, IndexSearchException, IteratorException 
	{
		// [ASantra: 1/7/2023] Remove the return statement and start your code.
		...
		KeyDataEntry keyDataEntry;
							...
							//  to find the first page and rid of keys
		leafPage = findRunStart(key, i_Rid);
		...
		keyDataEntry = leafPage.getCurrent(i_Rid);
	 ...
	 */
	 ...
	 public BTFileScan new_scan(KeyClass lo_key, KeyClass hi_key) throws IOException, KeyNotMatchException, IteratorException, ConstructPageException, PinPageException, UnpinPageException
	 {
